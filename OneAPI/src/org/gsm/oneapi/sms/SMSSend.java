...
/**
 * The main API interface for the OneAPI SMS Send methods
 */
public class SMSSend {
	...
	private static JSONRequest<SMSDeliveryReceiptSubscriptionResponse> smsDeliveryReceiptSubscriptionProcessor = new JSONRequest<SMSDeliveryReceiptSubscriptionResponse>(
			new SMSDeliveryReceiptSubscriptionResponse());
	 ...
	 * 
	 * @param endPoints
	 *            contains a set of service/ call specific endpoints
	 ...
	 */
	public SMSSend(ServiceEndpointsInterface endPoints, String authorisationHeader) {
	 ...
	 *            contains a set of service/ call specific endpoints
	 * @param username
	 *            is the account name allocated for use of the service
	 * @param password
	 *            is the corresponding authentication password
	 * @see org.gsm.oneapi.endpoints.ServiceEndpoints
	 */
	public SMSSend(ServiceEndpointsInterface endPoints, String username, String password) {
		String authorisationHeader = null;
	 ...
	 * 
	 * @param senderAddress
	 *            (mandatory) is the address to whom a responding SMS may be sent. Do not URL encode this value prior to passing to this function
	 * @param address
	 *            (mandatory) contains at least one address for each end user ID to send to; in this case their MSISDN including the 詔el:�protocol identifier and the country code preceded by �� i.e., tel:+16309700001. OneAPI also supports the Anonymous Customer Reference (ACR) if provided by the
	 *            operator.
	 * @param message
	 *            (mandatory) contains the message text to send.Messages over 160 characters may end up being sent as two or more messages by the operator.
	 ...
	 * @param senderName
	 *            (optional) is the name to appear on the user誷 terminal as the sender of the message
	 * @param notifyURL
	...
	public SMSSendResponse sendSMS(String senderAddress, String[] address, String message, String clientCorrelator, String notifyURL, String senderName, String callbackData) {
		...
		try {
			...
			clientCorrelator = new String(clientCorrelator.getBytes("iso-8859-1"), "utf-8");
		...
		for (String addr : address)
			formParameters.put("address", addr);
		String message1 = "This is a test for the UTF8 encoding. 电子邮件 Please let me know if this reaches you. Kind regards 【Sjoerd】";

		// message1 = "This is a test for the UTF8 encoding. 电子邮件 Please let me know if this reaches you. Kind regards 【Sjoerd】";
		...
		try {
			...
			//requestBody = "senderAddress=tel%3A%2B123456789&address=tel%3A%2B13671935968&address=&address=&address=&address=&message=%E5%8F%91%E8%87%AA%E6%88%91%E7%9A%84+iPad.+UTF8%3A+%E7%94%B5%E5%AD%90%E9%82%AE%E4%BB%B6.+%E3%80%90Yourzine%E3%80%91&clientCorrelator=123456&notifyURL=http%3A%2F%2Flocalhost%3A8080%2FOneAPI%2Fsmsdeliverynotification.jsp&senderName=%E3%80%90%E4%BC%98%E8%B5%9E%E3%80%91&callbackData=some-data-useful-to-the-requester";
	 ...
	 * @param requestId
	 *            (mandatory) contains the requestId returned from a previous call to the sendSMS function
	...
	public SMSSendDeliveryStatusResponse queryDeliveryStatus(String senderAddress, String requestId) {
		...
		if (senderAddress != null && requestId != null) {
			...
			try {
				...
				String endpoint = endPoints.getQuerySMSDeliveryEndpoint().replaceAll("\\{senderAddress\\}", URLEncoder.encode(senderAddress, "utf-8"))
						.replaceAll("\\{requestId\\}", URLEncoder.encode(requestId, "utf-8"));
	 ...
	 * @param clientCorrelator
	 *            (optional) uniquely identifies this subscription request. If there is a communication failure during the request, using the same clientCorrelator when retrying the request allows the operator to avoid setting up the same subscription twice
	 * @param callbackData
	 ...
	 *            (mandatory) contains the subscriptionId of a previously created SMS delivery receipt subscription
	 */
	 ...
	 * 
	 * @param request
	 *            the HttpServletRequest - make sure the input stream has not been read before calling
	...
	public static DeliveryInfoNotification convertDeliveryInfoNotification(HttpServletRequest request) {
		...
		if (request.getContentType() != null && request.getContentType().equalsIgnoreCase("application/json")) {
			try {
				...
				if (wrapper != null)
					deliveryInfoNotification = wrapper.getDeliveryInfoNotification();
